...
sealed class Heap<out A : Comparable<@UnsafeVariance A>> {
    ...
    operator
    fun plus(element: @UnsafeVariance A): Heap<A> = merge(this, Heap(element))

    fun <B> foldLeft(identity: B, f: (B) -> (A) -> B): B = unfold(this, { it.pop() }, identity, f)

    fun toList(): List<@UnsafeVariance A> = foldLeft(List(), { list: List<A> -> { e -> list.cons(e) } }).reverse()
    ...
    abstract class Empty<out A : Comparable<@UnsafeVariance A>> : Heap<A>() {
        ...
        override fun tail(): Result<Heap<A>> = Result(E)
        override fun get(index: Int): Result<A> = Result.failure(NoSuchElementException("Index out of bound"))
    ...
    companion object {
        ...
        operator
        fun <A : Comparable<A>> invoke(): Heap<A> = E

        operator
        fun <A : Comparable<A>> invoke(element: A): Heap<A> = H(1, E, element, E)
        ...
        protected fun <A : Comparable<A>> merge(head: A,
                                                first: Heap<A>,
